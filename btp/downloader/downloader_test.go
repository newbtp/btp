...
type downloadTester struct {
	...
	stateDb btpdb.Database // Database used by the tester for syncing from peers
	peerDb  btpdb.Database // Database of the peers containing all data
...
func (dl *downloadTester) HasHeader(hash common.Hash, number uint64) bool {
	return dl.GbtpeaderByHash(hash) != nil
}
...
// Gbtpeader retrieves a header from the testers canonical chain.
func (dl *downloadTester) GbtpeaderByHash(hash common.Hash) *types.Header {
	dl.lock.RLock()
	defer dl.lock.RUnlock()
...
// RequestBodies constructs a getBlockBodies mbtpod associated with a particular
...
// RequestReceipts constructs a getReceipts mbtpod associated with a particular
// peer in the download tester. The returned function can be used to retrieve
...
// RequestNodeData constructs a getNodeData mbtpod associated with a particular
// peer in the download tester. The returned function can be used to retrieve
...
func assertOwnChain(t *testing.T, tester *downloadTester, length int) {
	// Mark this mbtpod as a helper to report errors at callsite, not in here
	t.Helper()
...
func testSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	checkProgress(t, tester.downloader, "pristine", btpereum.SyncProgress{})
	...
	<-starting
	checkProgress(t, tester.downloader, "initial", btpereum.SyncProgress{
	...
	<-starting
	checkProgress(t, tester.downloader, "completing", btpereum.SyncProgress{
	...
	progress <- struct{}{}
	pending.Wait()
	checkProgress(t, tester.downloader, "final", btpereum.SyncProgress{
		StartingBlock: uint64(chain.len()/2 - 1),
...
func checkProgress(t *testing.T, d *Downloader, stage string, want btpereum.SyncProgress) {
	// Mark this mbtpod as a helper to report errors at callsite, not in here
	t.Helper()
...
func testForkedSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	<-starting
	checkProgress(t, tester.downloader, "forking", btpereum.SyncProgress{
