...
// PeerInfo represents a short summary of the btpereum sub-protocol metadata known
// about a connected peer.
type PeerInfo struct {
	Version    int      `json:"version"`    // btpereum protocol version negotiated
...
// Sbtpead updates the head hash and total difficulty of the peer.
func (p *peer) Sbtpead(hash common.Hash, td *big.Int) {
	p.lock.Lock()
	defer p.lock.Unlock()
...
// Handshake executes the btp protocol handshake, negotiating version number,
...
func (p *peer) String() string {
	return fmt.Sprintf("Peer %s [%s]", p.id,
		fmt.Sprintf("btp/%2d", p.version),
	)
}

// peerSet represents the collection of active peers currently participating in
// the btpereum sub-protocol.
type peerSet struct {
