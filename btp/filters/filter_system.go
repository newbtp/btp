...
// Package filters implements an btpereum filtering system for block,
...
func (sub *Subscription) Unsubscribe() {
	...
	uninstallLoop:
		for {
			...
			// the eventLoop broadcast mbtpod to deadlock when writing to the
			// filter event channel while the subscription loop is waiting for
			// this mbtpod to return (and thus not reading these events).
			select {
		...
		// will probably be closed by the client asap after this mbtpod returns.
		<-sub.Err()
	})
}
...
func (es *EventSystem) SubscribeLogs(crit btpereum.FilterQuery, logs chan []*types.Log) (*Subscription, error) {
...
func (es *EventSystem) subscribeMinedPendingLogs(crit btpereum.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribeLogs(crit btpereum.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribePendingLogs(crit btpereum.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
