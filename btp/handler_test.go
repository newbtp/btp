...
func testGetNodeData(t *testing.T, protocol int) {
	...
	generator := func(i int, block *core.BlockGen) {
		switch i {
		case 0:
			// In block 1, the test bank sends account #1 some btper.
			...
			block.AddTx(tx)
		case 1:
			// In block 2, the test bank sends some more btper to account #1.
...
// Tests that post btp protocol handshake, clients perform a mutual checkpoint
...
func testCheckpointChallenge(t *testing.T, syncmode downloader.SyncMode, checkpoint bool, timeout bool, empty bool, match bool, drop bool) {
	...
	blockchain, err := core.NewBlockChain(db, nil, config, btpash.NewFaker(), vm.Config{}, nil)
	if err != nil {
		t.Fatalf("failed to create new blockchain: %v", err)
	}
	pm, err := NewProtocolManager(config, cht, syncmode, DefaultConfig.NetworkId, new(event.TypeMux), new(testTxPool), btpash.NewFaker(), blockchain, db, 1, nil)
	if err != nil {
	...
	peer, _ := newTestPeer("peer", btp63, pm, true)
	defer peer.close()
...
func testBroadcastBlock(t *testing.T, totalPeers, broadcastExpected int) {
	...
	for i := 0; i < totalPeers; i++ {
		peer, _ := newTestPeer(fmt.Sprintf("peer %d", i), btp63, pm, true)
		defer peer.close()
		peers = append(peers, peer)
	}
	chain, _ := core.GenerateChain(gspec.Config, genesis, btpash.NewFaker(), db, 1, func(i int, gen *core.BlockGen) {})
