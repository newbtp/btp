...
import (
	...
	"github.com/btpereum/go-btpereum/tests"
)

// To generate a new callTracer test, copy paste the makeTest mbtpod below into
// a Gbtp console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = btp.getBlock(btp.getTransaction(tx).blockHash);
  var genesis = btp.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.btp.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  btp.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
