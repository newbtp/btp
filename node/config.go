...
import (
	...
	"github.com/btpereum/go-btpereum/accounts/external"
	"github.com/btpereum/go-btpereum/accounts/keystore"
	"github.com/btpereum/go-btpereum/accounts/scwallet"
	"github.com/btpereum/go-btpereum/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gbtp is "gbtp". If no
	...
	// registered services, instead those can use utility mbtpods to create/access
...
// ExtRPCEnabled returns the indicator whbtper node enables the external
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gbtp", keep that.
	if name == "gbtp" || name == "gbtp-testnet" {
		name = "Gbtp"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gbtp" instances.
var isOldGbtpResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gbtp 1.4 are used if they exist.
	if warn, isOld := isOldGbtpResource[path]; isOld {
		oldpath := ""
		if c.name() == "gbtp" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGbtpResourceWarning, "Using deprecated resource file %s, please move this file to the 'gbtp' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-btpereum-keystore")
		ephemeral = keydir
	}
