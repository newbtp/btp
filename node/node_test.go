...
// Tests whbtper services can be registered and duplicates caught.
...
func TestAPIGather(t *testing.T) {
	...
	makeAPI := func(result string) *OneMbtpodAPI {
		return &OneMbtpodAPI{fun: func() { calls <- result }}
	}
	services := map[string]struct {
		APIs  []rpc.API
		Maker InstrumentingWrapper
	...
	tests := []struct {
		...
		{"single_theOneMbtpod", "single.v1"},
		{"multi_theOneMbtpod", "multi.v1"},
		{"multi.v2_theOneMbtpod", "multi.v2"},
		{"multi.v2.nested_theOneMbtpod", "multi.v2.nested"},
	}
	for i, test := range tests {
		if err := client.Call(nil, test.Mbtpod); err != nil {
