...
// as well as utility mbtpods to operate on the service environment.
...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int, namespace string) (btpdb.Database, error) {
...
func (ctx *ServiceContext) OpenDatabaseWithFreezer(name string, cache int, handles int, freezer string, namespace string) (btpdb.Database, error) {
	if ctx.config.DataDir == "" {
		return rawdb.NewMemoryDatabase(), nil
	}
