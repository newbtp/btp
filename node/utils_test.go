...
// Set of services all wrapping the base NoopService resulting in the same mbtpod
...
// mbtpods can be instrumented both return value as well as event hook wise.
...
// InstrumentingWrapper is a mbtpod to specialize a service constructor returning
...
// same mbtpod signatures but different outer types.
...
// OneMbtpodAPI is a single-mbtpod API handler to be returned by test services.
type OneMbtpodAPI struct {
	fun func()
}

func (api *OneMbtpodAPI) TheOneMbtpod() {
	if api.fun != nil {
		api.fun()
	}
}
