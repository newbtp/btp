...
// header only chain.
func newCanonical(engine consensus.Engine, n int, full bool) (btpdb.Database, *BlockChain, error) {
	var (
	...
	blockchain, _ := NewBlockChain(db, nil, params.AllbtpashProtocolChanges, engine, vm.Config{}, nil)
...
func testFork(t *testing.T, blockchain *BlockChain, i, n int, full bool, comparator func(td1, td2 *big.Int)) {
	...
	db, blockchain2, err := newCanonical(btpash.NewFaker(), i, full)
	if err != nil {
	...
	if full {
		...
		hash1 = blockchain.GbtpeaderByNumber(uint64(i)).Hash()
		hash2 = blockchain2.GbtpeaderByNumber(uint64(i)).Hash()
	}
	if hash1 != hash2 {
	...
	if full {
		blockChainB = makeBlockChain(blockchain2.CurrentBlock(), n, btpash.NewFaker(), db, forkSeed)
		if _, err := blockchain2.InsertChain(blockChainB); err != nil {
			t.Fatalf("failed to insert forking chain: %v", err)
		}
	} else {
		headerChainB = makeHeaderChain(blockchain2.CurrentHeader(), n, btpash.NewFaker(), db, forkSeed)
...
func TestLastBlock(t *testing.T) {
	_, blockchain, err := newCanonical(btpash.NewFaker(), 0, true)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()

	blocks := makeBlockChain(blockchain.CurrentBlock(), 1, btpash.NewFullFaker(), blockchain.db, 0)
...
func testExtendCanonical(t *testing.T, full bool) {
	...
	_, processor, err := newCanonical(btpash.NewFaker(), length, full)
	if err != nil {
...
func testBrokenChain(t *testing.T, full bool) {
	...
	db, blockchain, err := newCanonical(btpash.NewFaker(), 10, full)
	if err != nil {
		t.Fatalf("failed to make new canonical chain: %v", err)
	}
	defer blockchain.Stop()
	...
	if full {
		chain := makeBlockChain(blockchain.CurrentBlock(), 5, btpash.NewFaker(), db, forkSeed)[1:]
		if err := testBlockChainImport(chain, blockchain); err == nil {
			t.Errorf("broken block chain not reported")
		}
	} else {
		chain := makeHeaderChain(blockchain.CurrentHeader(), 5, btpash.NewFaker(), db, forkSeed)[1:]
...
func testReorg(t *testing.T, first, second []int64, td int64, full bool) {
	...
	db, blockchain, err := newCanonical(btpash.NewFaker(), 0, full)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()
	...
	easyBlocks, _ := GenerateChain(params.TestChainConfig, blockchain.CurrentBlock(), btpash.NewFaker(), db, len(first), func(i int, b *BlockGen) {
		b.OffsetTime(first[i])
	})
	diffBlocks, _ := GenerateChain(params.TestChainConfig, blockchain.CurrentBlock(), btpash.NewFaker(), db, len(second), func(i int, b *BlockGen) {
	...
	if full {
		...
		for header := blockchain.GbtpeaderByNumber(blockchain.CurrentHeader().Number.Uint64() - 1); header.Number.Uint64() != 0; prev, header = header, blockchain.GbtpeaderByNumber(header.Number.Uint64()-1) {
...
func testBadHashes(t *testing.T, full bool) {
	...
	if full {
		blocks := makeBlockChain(blockchain.CurrentBlock(), 3, btpash.NewFaker(), db, 10)
		...
		headers := makeHeaderChain(blockchain.CurrentHeader(), 3, btpash.NewFaker(), db, 10)
...
func testReorgBadHashes(t *testing.T, full bool) {
	...
	headers := makeHeaderChain(blockchain.CurrentHeader(), 4, btpash.NewFaker(), db, 10)
	blocks := makeBlockChain(blockchain.CurrentBlock(), 4, btpash.NewFaker(), db, 10)
	...
	ncm, err := NewBlockChain(blockchain.db, nil, blockchain.chainConfig, btpash.NewFaker(), vm.Config{}, nil)
	if err != nil {
...
func testInsertNonceError(t *testing.T, full bool) {
	for i := 1; i < 25 && !t.Failed(); i++ {
		...
		if full {
			blocks := makeBlockChain(blockchain.CurrentBlock(), i, btpash.NewFaker(), db, 0)

			failAt = rand.Int() % len(blocks)
			failNum = blocks[failAt].NumberU64()

			blockchain.engine = btpash.NewFakeFailer(failNum)
			failRes, err = blockchain.InsertChain(blocks)
		} else {
			headers := makeHeaderChain(blockchain.CurrentHeader(), i, btpash.NewFaker(), db, 0)
		...
		for j := 0; j < i-failAt; j++ {
			if full {
				...
				if header := blockchain.GbtpeaderByNumber(failNum + uint64(j)); header != nil {
...
func TestFastVsFullChains(t *testing.T) {
	...
	blocks, receipts := GenerateChain(gspec.Config, genesis, btpash.NewFaker(), gendb, 1024, func(i int, block *BlockGen) {
	...
	archive, _ := NewBlockChain(archiveDb, nil, gspec.Config, btpash.NewFaker(), vm.Config{}, nil)
	defer archive.Stop()
	...
	fast, _ := NewBlockChain(fastDb, nil, gspec.Config, btpash.NewFaker(), vm.Config{}, nil)
	defer fast.Stop()
	...
	ancient, _ := NewBlockChain(ancientDb, nil, gspec.Config, btpash.NewFaker(), vm.Config{}, nil)
	defer ancient.Stop()
	...
	for i := 0; i < len(blocks); i++ {
		...
		if fheader, aheader := fast.GbtpeaderByHash(hash), archive.GbtpeaderByHash(hash); fheader.Hash() != aheader.Hash() {
			t.Errorf("block #%d [%x]: header mismatch: fastdb %v, archivedb %v", num, hash, fheader, aheader)
		}
		if anheader, arheader := ancient.GbtpeaderByHash(hash), archive.GbtpeaderByHash(hash); anheader.Hash() != arheader.Hash() {
...
// Tests that various import mbtpods move the chain head pointers to the correct
// positions.
func TestLightVsFastVsFullChainHeads(t *testing.T) {
	...
	height := uint64(1024)
	blocks, receipts := GenerateChain(gspec.Config, genesis, btpash.NewFaker(), gendb, int(height), nil)

	// makeDb creates a db instance for testing.
	makeDb := func() (btpdb.Database, func()) {
	...
	// Create a small assertion mbtpod to check the three heads
	...
	defer delfn()
	light, _ := NewBlockChain(lightDb, nil, gspec.Config, btpash.NewFaker(), vm.Config{}, nil)
...
func TestChainTxReorgs(t *testing.T) {
	...
	chain, _ := GenerateChain(gspec.Config, genesis, btpash.NewFaker(), db, 3, func(i int, gen *BlockGen) {
		switch i {
		case 0:
	...
	blockchain, _ := NewBlockChain(db, nil, gspec.Config, btpash.NewFaker(), vm.Config{}, nil)
	...
	chain, _ = GenerateChain(gspec.Config, genesis, btpash.NewFaker(), db, 5, func(i int, gen *BlockGen) {
		switch i {
		case 0:
...
func TestLogReorgs(t *testing.T) {
	...
	chain, _ := GenerateChain(params.TestChainConfig, genesis, btpash.NewFaker(), db, 2, func(i int, gen *BlockGen) {
		if i == 1 {
	...
	chain, _ = GenerateChain(params.TestChainConfig, genesis, btpash.NewFaker(), db, 3, func(i int, gen *BlockGen) {})
	if _, err := blockchain.InsertChain(chain); err != nil {
		t.Fatalf("failed to insert forked chain: %v", err)
	}
...
func TestLogRebirth(t *testing.T) {
	...
	// listenNewLog checks whbtper the received logs number is equal with expected.
	...
	forkChain, _ := GenerateChain(params.TestChainConfig, genesis, btpash.NewFaker(), db, 2, func(i int, gen *BlockGen) {
		if i == 1 {
			tx, err := types.SignTx(types.NewContractCreation(gen.TxNonce(addr1), new(big.Int), 1000000, new(big.Int), code), signer, key1)
			if err != nil {
				t.Fatalf("failed to create tx: %v", err)
	...
	newBlocks, _ := GenerateChain(params.TestChainConfig, chain[len(chain)-1], btpash.NewFaker(), db, 1, func(i int, gen *BlockGen) {})
	go listenNewLog(logsCh, 1)
...
func TestSideLogRebirth(t *testing.T) {
	...
	sideChain, _ := GenerateChain(params.TestChainConfig, genesis, btpash.NewFaker(), db, 2, func(i int, gen *BlockGen) {
		if i == 1 {
			tx, err := types.SignTx(types.NewContractCreation(gen.TxNonce(addr1), new(big.Int), 1000000, new(big.Int), code), signer, key1)
			if err != nil {
				t.Fatalf("failed to create tx: %v", err)
	...
	newBlocks, _ := GenerateChain(params.TestChainConfig, sideChain[len(sideChain)-1], btpash.NewFaker(), db, 1, func(i int, gen *BlockGen) {})
	go listenNewLog(logsCh, 1)
	if _, err := blockchain.InsertChain(newBlocks); err != nil {
		t.Fatalf("failed to insert forked chain: %v", err)
	}
...
func TestReorgSideEvent(t *testing.T) {
	...
	replacementBlocks, _ := GenerateChain(gspec.Config, genesis, btpash.NewFaker(), db, 4, func(i int, gen *BlockGen) {
...
func TestCanonicalBlockRetrieval(t *testing.T) {
	...
	chain, _ := GenerateChain(blockchain.chainConfig, blockchain.genesisBlock, btpash.NewFaker(), blockchain.db, 10, func(i int, gen *BlockGen) {})
...
func TestEIP155Transition(t *testing.T) {
	...
	blocks, _ := GenerateChain(gspec.Config, genesis, btpash.NewFaker(), db, 4, func(i int, block *BlockGen) {
		var (
	...
	blocks, _ = GenerateChain(config, blocks[len(blocks)-1], btpash.NewFaker(), db, 4, func(i int, block *BlockGen) {
		var (
			tx      *types.Transaction
			err     error
			basicTx = func(signer types.Signer) (*types.Transaction, error) {
...
func TestEIP161AccountRemoval(t *testing.T) {
	...
	blocks, _ := GenerateChain(gspec.Config, genesis, btpash.NewFaker(), db, 3, func(i int, block *BlockGen) {
		var (
			tx     *types.Transaction
			err    error
			signer = types.NewEIP155Signer(gspec.Config.ChainID)
...
//
// https://github.com/btpereum/go-btpereum/pull/15941
...
//
// Details at:
//  - https://github.com/btpereum/go-btpereum/issues/18977
//  - https://github.com/btpereum/go-btpereum/pull/18988
func TestLowDiffLongChain(t *testing.T) {
	...
	for number := head.NumberU64(); number > 0; number-- {
		if hash := chain.GbtpeaderByNumber(number).Hash(); hash != header.Hash() {
			t.Fatalf("header %d: canonical hash mismatch: have %x, want %x", number, hash, header.Hash())
		}
		header = chain.Gbtpeader(header.ParentHash, number-1)
	}
}
...
func TestPrunedImportSide(t *testing.T) {
	...
	//glogger.Verbosity(3)
	//log.Root().Sbtpandler(log.Handler(glogger))
...
func TestReorgToShorterRemovesCanonMapping(t *testing.T) {
	...
	if headerByNum := chain.GbtpeaderByNumber(canonNum); headerByNum != nil {
