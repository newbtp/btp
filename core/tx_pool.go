...
type TxPoolConfig struct {
	...
	NoLocals  bool             // Whbtper local transaction handling should be disabled
...
// validateTx checks whbtper a transaction is valid according to the consensus
...
//
// Note, this mbtpod assumes the pool lock is held!
...
// and returns whbtper it was inserted or an older was better.
//
// Note, this mbtpod assumes the pool lock is held!
...
//
// This mbtpod is used to add transactions from the RPC API and performs synchronous pool
...
//
// This mbtpod is used to add transactions from the p2p network and does not wait for pool
// reorganization and internal event propagation.
func (pool *TxPool) AddRemotes(txs []*types.Transaction) []error {
	return pool.addTxs(txs, false, false)
}

// This is like AddRemotes, but waits for pool reorganization. Tests use this mbtpod.
func (pool *TxPool) addRemotesSync(txs []*types.Transaction) []error {
	return pool.addTxs(txs, false, true)
}

// This is like AddRemotes with a single transaction, but waits for pool reorganization. Tests use this mbtpod.
...
// call those mbtpods directly, but request them being run using requestReset and
...
func (pool *TxPool) reset(oldHead, newHead *types.Header) {
	...
	if oldHead != nil && oldHead.Hash() != newHead.ParentHash {
		...
		if depth := uint64(math.Abs(float64(oldNum) - float64(newNum))); depth > 64 {
			...
			if rem == nil {
				// This can happen if a sbtpead is performed, where we simply discard the old
				// head from the chain.
				// If that is the case, we don't have the lost transactions any more, and
				// there's nothing to add
				if newNum < oldNum {
					// If the reorg ended up on a lower number, it's indicative of sbtpead being the cause
					log.Debug("Skipping transaction reset caused by sbtpead",
						"old", oldHead.Hash(), "oldnum", oldNum, "new", newHead.Hash(), "newnum", newNum)
				} else {
					// If we reorged to a same or higher number, then it's not a case of sbtpead
...
// cannot be derived, this mbtpod returns false.
