...
// genTxRing returns a block generator that sends btper in a ring
...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	var db btpdb.Database
	if !disk {
		db = rawdb.NewMemoryDatabase()
	} else {
		dir, err := ioutil.TempDir("", "btp-core-bench")
		if err != nil {
	...
	chain, _ := GenerateChain(gspec.Config, genesis, btpash.NewFaker(), db, b.N, gen)

	// Time the insertion of the new chain.
	// State and blocks are stored in the same DB.
	chainman, _ := NewBlockChain(db, nil, gspec.Config, btpash.NewFaker(), vm.Config{}, nil)
	defer chainman.Stop()
	b.ReportAllocs()
	b.ResetTimer()
...
// into a database.
func makeChainForBench(db btpdb.Database, full bool, count uint64) {
...
func benchWriteChain(b *testing.B, full bool, count uint64) {
	for i := 0; i < b.N; i++ {
		dir, err := ioutil.TempDir("", "btp-chain-bench")
		if err != nil {
			b.Fatalf("cannot create temporary directory: %v", err)
		}
...
func benchReadChain(b *testing.B, full bool, count uint64) {
	...
	for i := 0; i < b.N; i++ {
		...
		chain, err := NewBlockChain(db, nil, params.TestChainConfig, btpash.NewFaker(), vm.Config{}, nil)
		if err != nil {
			b.Fatalf("error creating chain: %v", err)
		}

		for n := uint64(0); n < count; n++ {
			header := chain.GbtpeaderByNumber(n)
			if full {
				hash := header.Hash()
