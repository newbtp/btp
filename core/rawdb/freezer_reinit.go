...
// of frozen ancient blocks. The mbtpod iterates over all the frozen blocks and
// injects into the database the block hash->number mappings and the transaction
// lookup entries.
func InitDatabaseFromFreezer(db btpdb.Database) error {
	...
	for i := uint64(0); i < frozen; i++ {
		...
		for !queue.Empty() {
			...
			if batch.ValueSize() > btpdb.IdealBatchSize || uint64(next) == frozen {
