...
//   reserving it for go-btpereum. This would also reduce the memory requirements
//   of Gbtp, and thus also GC overhead.
type freezer struct {
...
// HasAncient returns an indicator whbtper the specified ancient data exists
// in the freezer.
...
func (f *freezer) freeze(db btpdb.KeyValueStore) {
	...
	for {
		...
		// Log sombtping friendly for the user
		context := []interface{}{
