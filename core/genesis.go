...
func SetupGenesisBlock(db btpdb.Database, genesis *Genesis) (*params.ChainConfig, common.Hash, error) {
	if genesis != nil && genesis.Config == nil {
		return params.AllbtpashProtocolChanges, common.Hash{}, errGenesisNoConfig
	}
	...
	// Check whbtper the genesis block is already written.
	if genesis != nil {
		hash := genesis.ToBlock(nil).Hash()
		if hash != stored {
			return genesis.Config, hash, &GenesisMismatchError{stored, hash}
...
func (g *Genesis) configOrDefault(ghash common.Hash) *params.ChainConfig {
	...
	default:
		return params.AllbtpashProtocolChanges
	}
}

// ToBlock creates the genesis block and writes state of a genesis specification
// to the given database (or discards it if nil).
func (g *Genesis) ToBlock(db btpdb.Database) *types.Block {
	if db == nil {
...
func (g *Genesis) Commit(db btpdb.Database) (*types.Block, error) {
	block := g.ToBlock(db)
...
// The block is committed as the canonical head block.
func (g *Genesis) MustCommit(db btpdb.Database) *types.Block {
...
func GenesisBlockForTesting(db btpdb.Database, addr common.Address, balance *big.Int) *types.Block {
	g := Genesis{Alloc: GenesisAlloc{addr: {Balance: balance}}}
	return g.MustCommit(db)
}

// DefaultGenesisBlock returns the btpereum main net genesis block.
...
// DeveloperGenesisBlock returns the 'gbtp --dev' genesis block. Note, this must
// be seeded with the
