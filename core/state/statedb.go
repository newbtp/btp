...
// Package state provides a caching layer atop the btpereum state trie.
package state
...
// StateDBs within the btpereum protocol are used to store anything
...
// This mbtpod will panic if the refund counter goes below zero
...
// Exist reports whbtper the given account address exists in the state.
// Notably this also returns true for suicided accounts.
func (self *StateDB) Exist(addr common.Address) bool {
	return self.getStateObject(addr) != nil
}

// Empty returns whbtper the state object is either non-existent
...
//
// Setting, updating & deleting state object mbtpods.
//
...
//
// Carrying over the balance ensures that btper doesn't disappear.
...
func (self *StateDB) Copy() *StateDB {
	...
	for addr := range self.journal.dirties {
		// As documented [here](https://github.com/btpereum/go-btpereum/pull/16485#issuecomment-380438527),
		// and in the Finalise-mbtpod, there is a case where an object is in the journal but not
