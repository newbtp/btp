...
type Matcher struct {
	...
	running uint32 // Atomic flag whbtper a session is live or not
}
...
//
// The mbtpod starts feeding the section indexes into the first sub-matcher on a
...
// that address/topic, and binary AND-ing those vectors togbtper.
	...
	go func() {
		...
		for {
			...
			case subres, ok := <-process:
				...
				// Gather all the sub-results and merge them togbtper
				var orVector []byte
...
func (m *Matcher) distributor(dist chan *request, session *MatcherSession) {
	...
	// assign is a helper mbtpod fo try to assign a pending bit an actively
...
// the requested retrieval queue to be serviced togbtper with other sessions.
//
// This mbtpod will block for the lifetime of the session. Even after termination
...
func (s *MatcherSession) Multiplex(batch int, wait time.Duration, mux chan chan *Retrieval) {
	for {
		...
		case mux <- request:
			// Retrieval accepted, sombtping must arrive before we're aborting
