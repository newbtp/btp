...
// chainRetriever is used by the unconfirmed block set to verify whbtper a previously
// mined block is part of the canonical chain or not.
type chainRetriever interface {
	// GbtpeaderByNumber retrieves the canonical header associated with a block number.
...
func (set *unconfirmedBlocks) Shift(height uint64) {
	...
	for set.blocks != nil {
		...
		header := set.chain.GbtpeaderByNumber(next.index)
		switch {
		case header == nil:
		...
		default:
			// Block is not canonical, check whbtper we have an uncle or a lost block
			included := false
