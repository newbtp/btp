...
var (
	...
	retestbtpCommand = cli.Command{
		Action:      utils.MigrateFlags(retestbtp),
		Name:        "retestbtp",
		Usage:       "Launches gbtp in retestbtp mode",
		ArgsUsage:   "",
		Flags:       []cli.Flag{rpcPortFlag},
		Category:    "MISCELLANEOUS COMMANDS",
		Description: `Launches gbtp in retestbtp mode (no database, no network, only retestbtp RPC interface)`,
	}
)

type RetestbtpTestAPI interface {
...
type RetestbtpbtpAPI interface {
...
type RetestbtpDebugAPI interface {
...
type RetestbtpAPI struct {
	btpDb         btpdb.Database
	db            state.Database
...
func (api *RetestbtpAPI) SetChainParams(ctx context.Context, chainParams ChainParams) (bool, error) {
	// Clean up
	...
	chainConfig, genesisHash, err := core.SetupGenesisBlock(btpDb, genesis)
	if err != nil {
		return false, err
	}
	...
	case "NoProof", "NoReward":
		inner = btpash.NewFaker()
	case "btpash":
		inner = btpash.New(btpash.Config{
			CacheDir:       "btpash",
			CachesInMem:    2,
			CachesOnDisk:   3,
			DatasetsInMem:  1,
	...
	blockchain, err := core.NewBlockChain(btpDb, nil, chainConfig, engine, vm.Config{}, nil)
	if err != nil {
		return false, err
	}
	...
	api.db = state.NewDatabase(api.btpDb)
	api.blockNumber = 0
...
func (api *RetestbtpAPI) SendRawTransaction(ctx context.Context, rawTx hexutil.Bytes) (common.Hash, error) {
...
func (api *RetestbtpAPI) MineBlocks(ctx context.Context, number uint64) (bool, error) {
...
func (api *RetestbtpAPI) mineBlock() error {
	parentHash := rawdb.ReadCanonicalHash(api.btpDb, api.blockNumber)
	parent := rawdb.ReadBlock(api.btpDb, parentHash, api.blockNumber)
	...
	// If we are care about TheDAO hard-fork check whbtper to override the extra-data or not
	if daoBlock := api.chainConfig.DAOForkBlock; daoBlock != nil {
		// Check whbtper the block is among the fork extra-override range
		limit := new(big.Int).Add(daoBlock, params.DAOForkExtraRange)
		if header.Number.Cmp(daoBlock) >= 0 && header.Number.Cmp(limit) < 0 {
			// Depending whbtper we support or oppose the fork, override differently
...
func (api *RetestbtpAPI) importBlock(block *types.Block) error {
...
func (api *RetestbtpAPI) ModifyTimestamp(ctx context.Context, interval uint64) (bool, error) {
	api.blockInterval = interval
	return true, nil
}

func (api *RetestbtpAPI) ImportRawBlock(ctx context.Context, rawBlock hexutil.Bytes) (common.Hash, error) {
...
func (api *RetestbtpAPI) RewindToBlock(ctx context.Context, newHead uint64) (bool, error) {
	if err := api.blockchain.Sbtpead(newHead); err != nil {
		return false, err
	}
...
func (api *RetestbtpAPI) GetLogHash(ctx context.Context, txHash common.Hash) (common.Hash, error) {
	receipt, _, _, _ := rawdb.ReadReceipt(api.btpDb, txHash, api.chainConfig)
	if receipt == nil {
...
func (api *RetestbtpAPI) BlockNumber(ctx context.Context) (uint64, error) {
	//fmt.Printf("BlockNumber, response: %d\n", api.blockNumber)
	return api.blockNumber, nil
}

func (api *RetestbtpAPI) GetBlockByNumber(ctx context.Context, blockNr math.HexOrDecimal64, fullTx bool) (map[string]interface{}, error) {
...
func (api *RetestbtpAPI) AccountRangeAt(ctx context.Context,
	blockHashOrNumber *math.HexOrDecimal256, txIndex uint64,
	addressHash *math.HexOrDecimal256, maxResults uint64,
) (AccountRangeResult, error) {
	var (
	...
	if (*big.Int)(blockHashOrNumber).Cmp(big.NewInt(math.MaxInt64)) > 0 {
		...
		header = api.blockchain.GbtpeaderByHash(blockHash)
		...
		header = api.blockchain.GbtpeaderByNumber(blockNumber)
	...
	parentHeader := api.blockchain.GbtpeaderByHash(header.ParentHash)
...
func (api *RetestbtpAPI) GetBalance(ctx context.Context, address common.Address, blockNr math.HexOrDecimal64) (*math.HexOrDecimal256, error) {
	//fmt.Printf("GetBalance %x, block %d\n", address, blockNr)
	header := api.blockchain.GbtpeaderByNumber(uint64(blockNr))
...
func (api *RetestbtpAPI) GetCode(ctx context.Context, address common.Address, blockNr math.HexOrDecimal64) (hexutil.Bytes, error) {
	header := api.blockchain.GbtpeaderByNumber(uint64(blockNr))
	statedb, err := api.blockchain.StateAt(header.Root)
	if err != nil {
		return nil, err
...
func (api *RetestbtpAPI) GetTransactionCount(ctx context.Context, address common.Address, blockNr math.HexOrDecimal64) (uint64, error) {
	header := api.blockchain.GbtpeaderByNumber(uint64(blockNr))
	statedb, err := api.blockchain.StateAt(header.Root)
	if err != nil {
		return 0, err
...
func (api *RetestbtpAPI) StorageRangeAt(ctx context.Context,
	blockHashOrNumber *math.HexOrDecimal256, txIndex uint64,
	address common.Address,
	begin *math.HexOrDecimal256, maxResults uint64,
) (StorageRangeResult, error) {
...
func (api *RetestbtpAPI) ClientVersion(ctx context.Context) (string, error) {
	return "Gbtp-" + params.VersionWithCommit(gitCommit, gitDate), nil
}
...
func retestbtp(ctx *cli.Context) error {
	log.Info("Welcome to retestbtp!")
	// register signer API with server
	var (
		extapiURL = "n/a"
	)
	apiImpl := &RetestbtpAPI{}
	var testApi RetestbtpTestAPI = apiImpl
	var btpApi RetestbtpbtpAPI = apiImpl
	var debugApi RetestbtpDebugAPI = apiImpl
	...
	listener, _, err := rpc.StartHTTPEndpoint(httpEndpoint, rpcAPI, []string{"test", "btp", "debug", "web3"}, cors, vhosts, rpc.DefaultHTTPTimeouts)
	if err != nil {
