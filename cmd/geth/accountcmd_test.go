...
func TestAccountListEmpty(t *testing.T) {
	gbtp := runGbtp(t, "account", "list")
	gbtp.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gbtp := runGbtp(t, "account", "list", "--datadir", datadir)
	defer gbtp.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gbtp := runGbtp(t, "account", "new", "--lightkdf")
	defer gbtp.ExpectExit()
	gbtp.Expect(`
...
Your new account is locked with a password. Please give a password. Do not forget this password.
!! Unsupported terminal, password will be echoed.
Passphrase: {{.InputLine "sombtping"}}
Repeat passphrase: {{.InputLine "sombtping else"}}
...
func TestWalletImport(t *testing.T) {
	gbtp := runGbtp(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gbtp.ExpectExit()
	gbtp.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gbtp.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gbtp.StderrText(), m) {
...
// https://github.com/btpereum/go-btpereum/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gbtp.SetTemplateFunc("keypath", func(file string) string {
