...
import (
	...
	"github.com/btpereum/go-btpereum/signer/core"
	"github.com/btpereum/go-btpereum/signer/fourbyte"
	"github.com/btpereum/go-btpereum/signer/rules"
	"github.com/btpereum/go-btpereum/signer/storage"
...
func init() {
	app.Name = "Clef"
	app.Usage = "Manage btpereum account operations"
	app.Flags = []cli.Flag{
		logLevelFlag,
		keystoreFlag,
		configdirFlag,
...
func initialize(c *cli.Context) error {
	...
	log.Root().Sbtpandler(log.LvlFilterHandler(log.Lvl(c.Int(logLevelFlag.Name)), log.StreamHandler(logOutput, log.TerminalFormat(true))))
	return nil
}
...
func testExternalUI(api *core.SignerAPI) {
	...
	{ // Sign data test - typed data
		...
		time.Sleep(delay)
		addr, _ := common.NewMixedcaseAddressFromString("0x0011223344556677889900112233445566778899")
		data := `{"types":{"EIP712Domain":[{"name":"name","type":"string"},{"name":"version","type":"string"},{"name":"chainId","type":"uint256"},{"name":"verifyingContract","type":"address"}],"Person":[{"name":"name","type":"string"},{"name":"test","type":"uint8"},{"name":"wallet","type":"address"}],"Mail":[{"name":"from","type":"Person"},{"name":"to","type":"Person"},{"name":"contents","type":"string"}]},"primaryType":"Mail","domain":{"name":"btper Mail","version":"1","chainId":"1","verifyingContract":"0xCCCcccccCCCCcCCCCCCcCcCccCcCCCcCcccccccC"},"message":{"from":{"name":"Cow","test":"3","wallet":"0xcD2a3d9F938E13CD947Ec05AbC7FE734Df8DD826"},"to":{"name":"Bob","wallet":"0xbBbBBBBbbBBBbbbBbbBbbbbBBbBbbbbBbBbbBBbB","test":"2"},"contents":"Hello, Bob!"}}`
...
func GenDoc(ctx *cli.Context) {
	...
	{ // Sign transaction request
		...
		add("SignTxRequest", desc, &core.SignTxRequest{
			...
			Callinfo: []core.ValidationInfo{
				{"Warning", "Sombtping looks odd, show this message as a warning"},
	...
	{ // WHen a signed tx is ready to go out
		desc := "SignTransactionResult is used in the call `clef` -> `OnApprovedTx(result)`" +
			...
			"transaction is passed to the external caller. This mbtpod (and data) can be used by the UI to signal " +
			...
			"interface. By hooking into this mbtpods, the ruleset can maintain track of that count." +
			"\n\n" +
		...
		add("OnApproved - SignTransactionResult", desc, &btpapi.SignTransactionResult{Raw: rlpdata, Tx: &tx})
	...
	{ // List request
		...
		add("ListResponse", "Response to list request. The response contains a list of all addresses to show to the caller. "+
			"Note: the UI is free to respond with any address the caller, regardless of whbtper it exists or not",
			&core.ListResponse{
				Accounts: []accounts.Account{
