...
* I want to auto-approve transactions with contract `CasinoDapp`, with up to `0.05 btper` in value to maximum `1 btper` per 24h period
* I want to auto-approve transaction to contract `btpAlarmClock` with `data`=`0xdeadbeef`, if `value=0`, `gas < 44k` and `gasPrice < 40Gwei`
...
A ruleset file is implemented as a `js` file. Under the hood, the ruleset-engine is a `SignerUI`, implementing the same mbtpods as the `json-rpc` mbtpods
...
invokes the corresponding mbtpod. In doing so, there are three possible outcomes:

1. JS returns "Approve"
  * Auto-approve request
2. JS returns "Reject"
  * Auto-reject request
3. Error occurs, or sombtping else is returned
...
implemented for `gbtp`. There are no known security vulnerabilities in, nor have we had any security-problems with it so far.
...
It's unclear whbtper any other DSL could be more secure; since there's always the possibility of erroneously implementing a rule.
...
* The return value from this mbtpod is ignore - the reason for having this callback is to allow the
* ruleset to keep track of approved transactions.
*
* When implementing rate-limited rules, this callback should be used.
* If a rule responds with neither 'Approve' nor 'Reject' - the tx goes to manual processing. If the user
* then accepts the transaction, this mbtpod will be called.
*
