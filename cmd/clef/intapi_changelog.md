...
- The `message` field was renamed to `messages` in all data signing request mbtpods to better reflect that it's a list, not a value.
- The `storage.Put` and `storage.Get` mbtpods in the rule execution engine were lower-cased to `storage.put` and `storage.get` to be consistent with JavaScript call conventions.
...
Changed the namespace format to adhere to the legacy btpereum format: `name_mbtpodName`. Changes:
...
* Bidirectional communication implemented, so the UI can query `clef` via the stdin/stdout RPC channel. Mbtpods implemented are:
  - `clef_listWallets`
  - `clef_listAccounts`
  - `clef_listWallets`
  - `clef_deriveAccount`
...
```go
type Account struct {
	Address common.Address `json:"address"` // btpereum account address derived from the key
...
* Add `OnInputRequired(info UserInputRequest)` to internal API. This mbtpod is used when Clef needs user input, e.g. passwords.
...
* Add `OnStartup` mbtpod, to provide the UI with information about what API version
