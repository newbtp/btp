...
import (
	...
	"github.com/btpereum/go-btpereum/contracts/checkpointoracle"
	"github.com/btpereum/go-btpereum/contracts/checkpointoracle/contract"
...
func sign(ctx *cli.Context) error {
	var (
		offline bool // The indicator whbtper we sign checkpoint by offline.
		chash   common.Hash
		cindex  uint64
	...
	if !ctx.GlobalIsSet(nodeURLFlag.Name) {
		...
		head, err := btpclient.NewClient(node).HeaderByNumber(reqCtx, nil)
		if err != nil {
			return err
		}
	...
	// isAdmin checks whbtper the specified signer is admin.
...
func publish(ctx *cli.Context) error {
	...
	head, err := btpclient.NewClient(client).HeaderByNumber(reqCtx, nil)
	if err != nil {
		return err
	}
	num := head.Number.Uint64()
	recent, err := btpclient.NewClient(client).HeaderByNumber(reqCtx, big.NewInt(int64(num-128)))
	if err != nil {
		return err
	}
