...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many btpers to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.btpstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.btpstats = w.readString() + ":" + w.conf.btpstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.btpstats)
		infos.node.btpstats = w.readDefaultString(infos.node.btpstats) + ":" + w.conf.btpstats
	}
