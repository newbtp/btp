...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.btpstats == "" {
		log.Error("No btpstats server configured")
		return
	}
	...
	if w.conf.Genesis.Config.btpash != nil && !boot {
		fmt.Println()
		if infos.btpashdir == "" {
			fmt.Printf("Where should the btpash mining DAGs be stored on the remote machine?\n")
			infos.btpashdir = w.readString()
		} else {
			fmt.Printf("Where should the btpash mining DAGs be stored on the remote machine? (default = %s)\n", infos.btpashdir)
			infos.btpashdir = w.readDefaultString(infos.btpashdir)
		}
	}
	...
	if infos.btpstats == "" {
		...
		infos.btpstats = w.readString() + ":" + w.conf.btpstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.btpstats)
		infos.btpstats = w.readDefaultString(infos.btpstats) + ":" + w.conf.btpstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.btpash != nil {
			// btpash based miners only need an btperbase to mine against
			fmt.Println()
			if infos.btperbase == "" {
				fmt.Printf("What address should the miner use?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.btperbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.btperbase)
				infos.btperbase = w.readDefaultAddress(common.HexToAddress(infos.btperbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy btpereum node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
