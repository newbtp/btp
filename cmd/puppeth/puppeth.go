...
// puppbtp is a command to assemble and maintain private networks.
package main
...
func main() {
	app := cli.NewApp()
	app.Name = "puppbtp"
	app.Usage = "assemble and maintain private btpereum networks"
	app.Flags = []cli.Flag{
		cli.StringFlag{
			Name:  "network",
	...
	app.Before = func(c *cli.Context) error {
		...
		log.Root().Sbtpandler(log.LvlFilterHandler(log.Lvl(c.Int("loglevel")), log.StreamHandler(os.Stdout, log.TerminalFormat(true))))
