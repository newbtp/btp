...
// Tests the go-btpereum to Albtp chainspec conversion for the Stureby testnet.
func TestAlbtpSturebyConverter(t *testing.T) {
	blob, err := ioutil.ReadFile("testdata/stureby_gbtp.json")
	if err != nil {
	...
	spec, err := newAlbtpGenesisSpec("stureby", &genesis)
	if err != nil {
		t.Fatalf("failed creating chainspec: %v", err)
	}

	expBlob, err := ioutil.ReadFile("testdata/stureby_albtp.json")
	if err != nil {
		t.Fatalf("could not read file: %v", err)
	}
	expspec := &albtpGenesisSpec{}
...
// Tests the go-btpereum to Parity chainspec conversion for the Stureby testnet.
