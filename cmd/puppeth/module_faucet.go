...
FROM btpereum/client-go:alltools-latest
...
ENTRYPOINT [ \
	"faucet", "--genesis", "/genesis.json", "--network", "{{.NetworkID}}", "--bootnodes", "{{.Bootnodes}}", "--btpstats", "{{.btpstats}}", "--btpport", "{{.btpPort}}",     \
...
func (info *faucetInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Funding amount (base tier)":   fmt.Sprintf("%d btpers", info.amount),
...
// checkFaucet does a health-check against a faucet server to verify whbtper
