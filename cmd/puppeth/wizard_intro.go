...
// makeWizard creates and returns a new puppbtp wizard.
...
// setting up a new or managing an existing btpereum private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to puppbtp, your btpereum private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new btpereum network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and btpstats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Puppbtp uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering btpereum network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".puppbtp", w.network)
