...
func (w *wizard) makeGenesis() {
	...
	fmt.Println(" 1. btpash - proof-of-work")
	fmt.Println(" 2. Clique - proof-of-authority")

	choice := w.read()
	switch {
	case choice == "1":
		// In case of btpash, we're pretty much done
		genesis.Config.btpash = new(params.btpashConfig)
...
// importGenesis imports a Gbtp genesis spec into puppbtp.
...
func (w *wizard) manageGenesis() {
	// Figure out whbtper to modify or export the genesis
	fmt.Println()
	...
	case "2":
		...
		fmt.Printf("  Will create %s.json, %s-albtp.json, %s-harmony.json, %s-parity.json\n", w.network, w.network, w.network, w.network)
		...
		// Export the native genesis spec used by puppbtp and Gbtp
		gbtpJson := filepath.Join(folder, fmt.Sprintf("%s.json", w.network))
		if err := ioutil.WriteFile((gbtpJson), out, 0644); err != nil {
			log.Error("Failed to save genesis file", "err", err)
			return
		}
		log.Info("Saved native genesis chain spec", "path", gbtpJson)

		// Export the genesis spec used by Albtp (formerly C++ btpereum)
		if spec, err := newAlbtpGenesisSpec(w.network, w.conf.Genesis); err != nil {
			log.Error("Failed to create Albtp chain spec", "err", err)
		} else {
			saveGenesis(folder, w.network, "albtp", spec)
		}
		...
		// Export the genesis spec used by Harmony (formerly btpereumJ
	...
	case "3":
		...
		w.conf.flush()
	default:
		log.Error("That's not sombtping I can do")
		return
	}
}
