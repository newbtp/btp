...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"btpstats", "explorer", "wallet", "faucet"} {
	...
	// If we have btpstats running, ask whbtper to make the secret public or not
	if w.conf.btpstats != "" {
		fmt.Println()
		fmt.Println("Include btpstats secret on dashboard (y/n)? (default = yes)")
