...
FROM puppbtp/blockscout:latest

ADD genesis.json /genesis.json
RUN \
  echo 'gbtp --cache 512 init /genesis.json' > explorer.sh && \
  echo $'gbtp --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.btpPort}} --bootnodes {{.Bootnodes}} --btpstats \'{{.btpstats}}\' --cache=512 --rpc --rpcapi "net,web3,btp,shh,debug" --rpccorsdomain "*" --rpcvhosts "*" --ws --wsorigins "*" --exitwhensynced' >> explorer.sh && \
  echo $'exec gbtp --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.btpPort}} --bootnodes {{.Bootnodes}} --btpstats \'{{.btpstats}}\' --cache=512 --rpc --rpcapi "net,web3,btp,shh,debug" --rpccorsdomain "*" --rpcvhosts "*" --ws --wsorigins "*" &' >> explorer.sh && \
...
services:
    explorer:
        ...
        ports:
            - "{{.btpPort}}:{{.btpPort}}"
            - "{{.btpPort}}:{{.btpPort}}/udp"{{if not .VHost}}
            - "{{.WebPort}}:4000"{{end}}
        environment:
            - btp_PORT={{.btpPort}}
            - btp_NAME={{.btpName}}
...
func deployExplorer(client *sshClient, network string, bootnodes []string, config *explorerInfos, nocache bool, isClique bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(explorerDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"btpstats":  config.node.btpstats,
		"btpPort":   config.node.port,
	})
	...
	template.Must(template.New("").Parse(explorerComposefile)).Execute(composefile, map[string]interface{}{
		...
		"DBDir":       config.dbdir,
		"btpPort":     config.node.port,
		"btpName":     config.node.btpstats[:strings.Index(config.node.btpstats, ":")],
		"WebPort":     config.port,
...
func (info *explorerInfos) Report() map[string]string {
	report := map[string]string{
		...
		"btpereum listener port ": strconv.Itoa(info.node.port),
		"btpstats username":       info.node.btpstats,
	}
	return report
}
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	p2pPort := infos.portmap[infos.envvars["btp_PORT"]+"/tcp"]
	...
	stats := &explorerInfos{
		node: &nodeInfos{
			datadir:  infos.volumes["/opt/app/.btpereum"],
			port:     infos.portmap[infos.envvars["btp_PORT"]+"/tcp"],
			btpstats: infos.envvars["btp_NAME"],
		},
