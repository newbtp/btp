...
FROM puppbtp/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gbtp --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec gbtp --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --btpstats \'{{.btpstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh

RUN \
	sed -i 's/PuppbtpNetworkID/{{.NetworkID}}/g' dist/js/btperwallet-master.js && \
	sed -i 's/PuppbtpNetwork/{{.Network}}/g'     dist/js/btperwallet-master.js && \
	sed -i 's/PuppbtpDenom/{{.Denom}}/g'         dist/js/btperwallet-master.js && \
	sed -i 's/PuppbtpHost/{{.Host}}/g'           dist/js/btperwallet-master.js && \
	sed -i 's/PuppbtpRPCPort/{{.RPCPort}}/g'     dist/js/btperwallet-master.js
...
services:
  wallet:
    ...
    ports:
      ...
      - "{{.WebPort}}:80"{{end}}
    volumes:
      - {{.Datadir}}:/root/.btpereum
    environment:
      - NODE_PORT={{.NodePort}}/tcp
      - STATS={{.btpstats}}{{if .VHost}}
      - VIRTUAL_HOST={{.VHost}}
      - VIRTUAL_PORT=80{{end}}
    logging:
      driver: "json-file"
...
// checkWallet does a health-check against web wallet server to verify whbtper
// it's running, and if yes, whbtper it's responsive.
