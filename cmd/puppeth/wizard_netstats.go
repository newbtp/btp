...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for btpstats availability")
	if infos, err := checkbtpstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["btpstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["btpstats"] = infos.Report()
		btpstats = infos.config
	}
...
func (stats serverStats) render() {
	...
	table.Sbtpeader([]string{"Server", "Address", "Service", "Config", "Value"})
