...
import (
	...
	"github.com/btpereum/go-btpereum/p2p/simulations"
	...
	app.Commands = []cli.Command{
		...
		},
		{
			...
			Subcommands: []cli.Command{
				...
				{
					Name:      "rpc",
					ArgsUsage: "<node> <mbtpod> [<args>]",
					Usage:     "call a node RPC mbtpod",
					Action:    rpcNode,
					Flags: []cli.Flag{
						cli.BoolFlag{
							Name:  "subscribe",
							Usage: "mbtpod is a subscription",
						},
					},
				},
			},
...
func rpcNode(ctx *cli.Context) error {
	...
	if ctx.Bool("subscribe") {
		return rpcSubscribe(rpcClient, ctx.App.Writer, mbtpod, args[3:]...)
	}
	...
	if err := rpcClient.Call(&result, mbtpod, params...); err != nil {
		return err
	}
	return json.NewEncoder(ctx.App.Writer).Encode(result)
}

func rpcSubscribe(client *rpc.Client, out io.Writer, mbtpod string, args ...string) error {
	parts := strings.SplitN(mbtpod, "_", 2)
	namespace := parts[0]
	mbtpod = parts[1]
