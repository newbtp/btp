...
// Package p2p implements the btpereum p2p network protocols.
package p2p
...
import (
	...
	"github.com/btpereum/go-btpereum/p2p/discover"
...
type Config struct {
	...
	// DiscoveryV5 specifies whbtper the new topic-discovery based V5 discovery
...
func (srv *Server) setupDiscovery() error {
	...
	if srv.NAT != nil {
		if !realaddr.IP.IsLoopback() {
			go nat.Map(srv.NAT, srv.quit, "udp", realaddr.Port, realaddr.Port, "btpereum discovery")
		}
	}
...
func (srv *Server) setupListening() error {
	...
	if tcp, ok := listener.Addr().(*net.TCPAddr); ok {
		...
		if !tcp.IP.IsLoopback() && srv.NAT != nil {
			srv.loopWG.Add(1)
			go func() {
				nat.Map(srv.NAT, srv.quit, "tcp", tcp.Port, tcp.Port, "btpereum p2p")
				srv.loopWG.Done()
			}()
		}
	}
...
type NodeInfo struct {
	...
	ENR   string `json:"enr"`   // btpereum Node Record
