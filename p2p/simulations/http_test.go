...
func init() {
	...
	log.Root().Sbtpandler(log.LvlFilterHandler(log.Lvl(*loglevel), log.StreamHandler(colorable.NewColorableStderr(), log.TerminalFormat(true))))
}
...
// TestHTTPNodeRPC tests calling RPC mbtpods on nodes via the HTTP API
func TestHTTPNodeRPC(t *testing.T) {
	...
	// call some RPC mbtpods using client 2
	if err := rpcClient2.CallContext(ctx, nil, "test_add", 10); err != nil {
		t.Fatalf("error calling RPC mbtpod: %s", err)
	}
	var result int64
