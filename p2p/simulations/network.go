...
import (
	...
	"github.com/btpereum/go-btpereum/p2p/simulations/adapters"
)
...
// starting nodes and connecting them togbtper.
//
...
// Connect connects two nodes togbtper by calling the "admin_addPeer" RPC
// mbtpod on the "one" node so that it connects to the "other" node
...
// mbtpod on the "one" node so that it disconnects from the "other" node
...
// NOTE:
// it also checks whbtper there has been recent attempt to connect the peers
...
func (net *Network) Shutdown() {
	for _, node := range net.Nodes {
		...
		// If the node has the close mbtpod, call it.
...
type Node struct {
	...
	// up tracks whbtper or not the node is running
	up   bool
	upMu sync.RWMutex
}
...
func (n *Node) UnmarshalJSON(raw []byte) error {
	...
	// Ticket: https://github.com/btpersphere/go-btpereum/issues/1177
	node := struct {
		Config *adapters.NodeConfig `json:"config,omitempty"`
		Up     bool                 `json:"up"`
	}{}
...
type Conn struct {
	...
	// Up tracks whbtper or not the connection is active
	Up bool `json:"up"`
...
// nodesUp returns whbtper both nodes are currently up
