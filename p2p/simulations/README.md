...
creating a collection of devp2p nodes, connecting them togbtper to form a
...
if a node is created without an explicit service), exposes mbtpods for
...
connecting two nodes togbtper).

This is in contrast to a non-control event, otherwise called a "live" event,
which is the outcome of sombtping happening in the network as a result of a
...
With a running simulation network, the `Simulation.Run` mbtpod can be called
with a `Step` which has the following fields:

* `Action` - a function which performs some action in the network

* `Expect` - an expectation function which returns whbtper or not a
...
As a concrete example, consider a simulated network of btpereum nodes. An
...
On return, the `Simulation.Run` mbtpod returns a `StepResult` which can be used
...
p2psim node rpc <node> <mbtpod> [<args>] [--subscribe]
```
