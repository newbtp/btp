...
const (
	...
	ntpFailurbtpreshold = 32               // Continuous timeouts after which to check NTP
...
type packetV4 interface {
	// preverify checks whbtper the packet is valid and should be handled at all.
...
type replyMatcher struct {
	...
	// reply was acceptable. The second return value indicates whbtper the callback should
...
type reply struct {
	...
	ip   net.IP
	data packetV4
	// loop indicates whbtper there was
...
// whbtper any matcher considered the packet acceptable.
...
func (t *UDPv4) loop() {
	...
	for {
		...
		case r := <-t.gotreply:
			var matched bool // whbtper any replyMatcher considered the reply acceptable.
			for el := plist.Front(); el != nil; el = el.Next() {
		...
		case now := <-timeout.C:
			...
			if contTimeouts > ntpFailurbtpreshold {
...
// expired checks whbtper the given UNIX time stamp is in the past.
