...
// Less reports whbtper the element with
...
func (s *ticketStore) adjustWithTicket(now mclock.AbsTime, targbtpash common.Hash, t *ticket) {
	for i, topic := range t.topics {
		if tt, ok := s.radius[topic]; ok {
			tt.adjustWithTicket(now, targbtpash, ticketRef{t, i})
		}
	}
}
...
func (r *topicRadius) adjustWithTicket(now mclock.AbsTime, targbtpash common.Hash, t ticketRef) {
	...
	r.adjust(now, targbtpash, t.t.node.sha, inside)
}

func (r *topicRadius) adjust(now mclock.AbsTime, targbtpash, addrHash common.Hash, inside float64) {
	...
	delete(r.buckets[bucket].lookupSent, targbtpash)
}
