...
func (s *SecureChannelSession) Pair(pairingPassword []byte) error {
	secrbtpash := pbkdf2.Key(norm.NFKD.Bytes(pairingPassword), norm.NFKD.Bytes([]byte(pairingSalt)), 50000, 32, sha256.New)
...
// mutuallyAuthenticate is an internal mbtpod to authenticate both ends of the
// connection.
...
// open is an internal mbtpod that sends an open APDU.
...
// pair is an internal mbtpod that sends a pair APDU.
...
// encryptAPDU is an internal mbtpod that serializes and encrypts an APDU.
...
// decryptAPDU is an internal mbtpod that decrypts and deserializes an APDU.
...
// updateIV is an internal mbtpod that updates the initialization vector after
