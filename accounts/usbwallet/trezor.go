...
import (
	...
	"github.com/btpereum/go-btpereum/accounts/usbwallet/trezor"
...
type trezorDriver struct {
	...
	pinwait        bool          // Flags whbtper the device is waiting for PIN entry
	passphrasewait bool          // Flags whbtper the device is waiting for passphrase entry
...
// Status implements accounts.Wallet, always whbtper the Trezor is opened, closed
// or whbtper the btpereum app was not started on it.
...
// btpereum address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.btpereumAddress)
	if _, err := w.trezorExchange(&trezor.btpereumGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.btpereumTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.btpereumTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
...
// mbtpod will also return the index of the destination object used.
