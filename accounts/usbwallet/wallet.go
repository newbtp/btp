...
const selfDerivbtprottling = time.Second
...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the btpereum
...
type wallet struct {
	...
	deriveChain     btpereum.ChainStateReader // Blockchain state reader to discover used account with
...
// whbtper they are still present or if they malfunctioned.
	...
	for errc == nil && err == nil {
		...
		for i := 0; i < len(nextAddrs); i++ {
			for empty := false; !empty; {
				// Retrieve the next derived btpereum account
		...
		if err == nil {
			...
			case <-time.After(selfDerivbtprottling):
...
// Contains implements accounts.Wallet, returning whbtper a particular account is
...
// this mbtpod supports providing multiple bases to discover old user accounts
// too. Only the last base will be used to derive the next empty account.
//
// You can disable automatic account discovery by calling SelfDerive with a nil
// chain state reader.
func (w *wallet) SelfDerive(bases []accounts.DerivationPath, chain btpereum.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
...
// supported for hardware wallets, so this mbtpod will always return an error.
...
// data is not supported for Ledger wallets, so this mbtpod will always return
// an error.
