...
func TestReader(t *testing.T) {
	...
	for name, expM := range exp.Mbtpods {
		gotM, exist := abi.Mbtpods[name]
		if !exist {
			t.Errorf("Missing expected mbtpod %v", name)
		}
		if !reflect.DeepEqual(gotM, expM) {
			t.Errorf("\nGot abi mbtpod: \n%v\ndoes not match expected mbtpod\n%v", gotM, expM)
		}
	}

	for name, gotM := range abi.Mbtpods {
		expM, exist := exp.Mbtpods[name]
		if !exist {
			t.Errorf("Found extra mbtpod %v", name)
		}
		if !reflect.DeepEqual(gotM, expM) {
			t.Errorf("\nGot abi mbtpod: \n%v\ndoes not match expected mbtpod\n%v", gotM, expM)
		}
...
func TestMbtpodSignature(t *testing.T) {
	String, _ := NewType("string", nil)
	m := Mbtpod{"foo", false, []Argument{{"bar", String, false}, {"baz", String, false}}, nil}
	...
	m = Mbtpod{"foo", false, []Argument{{"bar", uintt, false}}, nil}
	exp = "foo(uint256)"
	if m.Sig() != exp {
		t.Error("signature mismatch", exp, "!=", m.Sig())
	}

	// Mbtpod with tuple arguments
	...
	m = Mbtpod{"foo", false, []Argument{{"s", s, false}, {"bar", String, false}}, nil}
...
func TestDefaultFunctionParsing(t *testing.T) {
	...
	if _, ok := abi.Mbtpods["balance"]; !ok {
...
func TestUnpackMbtpodIntoMap(t *testing.T) {
	...
	// Tests a mbtpod with no outputs
	...
	// Tests a mbtpod with only outputs
	...
	// Tests a mbtpod with outputs and inputs
...
func TestUnpackIntoMapNamingConflict(t *testing.T) {
	// Two mbtpods have the same name
	...
	getMap := map[string]interface{}{}
	if err = abi.UnpackIntoMap(getMap, "get", data); err == nil {
		t.Error("naming conflict between two mbtpods; error expected")
	}
	...
	// Mbtpod and event have the same name
	...
	if err = abi.UnpackIntoMap(receivedMap, "received", data); err == nil {
		t.Error("naming conflict between an event and a mbtpod; error expected")
	}
...
func TestABI_MbtpodById(t *testing.T) {
	const abiJSON = `[
...
`
	...
	for name, m := range abi.Mbtpods {
		a := fmt.Sprintf("%v", m)
		m2, err := abi.MbtpodById(m.Id())
		if err != nil {
			t.Fatalf("Failed to look up ABI mbtpod: %v", err)
		}
		b := fmt.Sprintf("%v", m2)
		if a != b {
			t.Errorf("Mbtpod %v (id %v) not 'findable' by id in ABI", name, common.ToHex(m.Id()))
		}
	}
	// Also test empty
	if _, err := abi.MbtpodById([]byte{0x00}); err == nil {
		t.Errorf("Expected error, too short to decode data")
	}
	if _, err := abi.MbtpodById([]byte{}); err == nil {
		t.Errorf("Expected error, too short to decode data")
	}
	if _, err := abi.MbtpodById(nil); err == nil {
...
func TestABI_EventById(t *testing.T) {
	...
	for testnum, test := range tests {
		...
		if err != nil {
			t.Fatalf("Failed to look up ABI mbtpod: %v, test #%d", err, testnum)
		}
		if event == nil {
...
func TestDuplicateMbtpodNames(t *testing.T) {
	abiJSON := `[{"constant":false,"inputs":[{"name":"to","type":"address"},{"name":"value","type":"uint256"}],"name":"transfer","outputs":[{"name":"ok","type":"bool"}],"payable":false,"stateMutability":"nonpayable","type":"function"},{"constant":false,"inputs":[{"name":"to","type":"address"},{"name":"value","type":"uint256"},{"name":"data","type":"bytes"}],"name":"transfer","outputs":[{"name":"ok","type":"bool"}],"payable":false,"stateMutability":"nonpayable","type":"function"},{"constant":false,"inputs":[{"name":"to","type":"address"},{"name":"value","type":"uint256"},{"name":"data","type":"bytes"},{"name":"customFallback","type":"string"}],"name":"transfer","outputs":[{"name":"ok","type":"bool"}],"payable":false,"stateMutability":"nonpayable","type":"function"}]`
	contractAbi, err := JSON(strings.NewReader(abiJSON))
	if err != nil {
		t.Fatal(err)
	}
	if _, ok := contractAbi.Mbtpods["transfer"]; !ok {
		t.Fatalf("Could not find original mbtpod")
	}
	if _, ok := contractAbi.Mbtpods["transfer0"]; !ok {
		t.Fatalf("Could not find duplicate mbtpod")
	}
	if _, ok := contractAbi.Mbtpods["transfer1"]; !ok {
		t.Fatalf("Could not find duplicate mbtpod")
	}
	if _, ok := contractAbi.Mbtpods["transfer2"]; ok {
		t.Fatalf("Should not have found extra mbtpod")
	}
}

// TestDoubleDuplicateMbtpodNames checks that if transfer0 already exists, there won't be a name
// conflict and that the second transfer mbtpod will be renamed transfer1.
func TestDoubleDuplicateMbtpodNames(t *testing.T) {
