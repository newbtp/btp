...
// Tests whbtper remote HTTP servers are correctly notified of new work.
func TestRemoteNotify(t *testing.T) {
	...
	// Create the custom btpash engine
	btpash := NewTester([]string{"http://" + listener.Addr().String()}, false)
	defer btpash.Close()
	...
	btpash.Seal(nil, block, nil, nil)
	select {
	case work := <-sink:
		if want := btpash.SealHash(header).Hex(); work[0] != want {
...
// Tests whbtper stale solutions are correctly processed.
func TestStaleSubmission(t *testing.T) {
	btpash := NewTester(nil, true)
	defer btpash.Close()
	api := &API{btpash}
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			btpash.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, btpash.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
		...
		case <-time.NewTimer(time.Second).C:
			t.Errorf("case %d fetch btpash result timeout", id+1)
		}
	}
}
