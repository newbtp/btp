...
// Tests that btpash works correctly in test mode.
func TestTestMode(t *testing.T) {
	header := &types.Header{Number: big.NewInt(1), Difficulty: big.NewInt(100)}

	btpash := NewTester(nil, false)
	defer btpash.Close()

	results := make(chan *types.Block)
	err := btpash.Seal(nil, types.NewBlockWithHeader(header), results, nil)
	if err != nil {
	...
	case block := <-results:
		...
		if err := btpash.VerifySeal(nil, header); err != nil {
...
// It reproduces https://github.com/btpereum/go-btpereum/issues/14943
func TestCacheFileEvict(t *testing.T) {
	tmpdir, err := ioutil.TempDir("", "btpash-test")
	if err != nil {
		t.Fatal(err)
	}
...
func verifyTest(wg *sync.WaitGroup, e *btpash, workerIndex, epochs int) {
	defer wg.Done()
...
func TestRemoteSealer(t *testing.T) {
	...
	sealhash := btpash.SealHash(header)

	// Push new work.
	results := make(chan *types.Block)
	btpash.Seal(nil, block, results, nil)
...
func TestHashRate(t *testing.T) {
	...
	if tot := btpash.Hashrate(); tot != float64(expect) {
...
func TestClosedRemoteSealer(t *testing.T) {
	...
	api := &API{btpash}
	if _, err := api.GetWork(); err != errbtpashStopped {
		t.Error("expect to return an error to indicate btpash is stopped")
	}

	if res := api.SubmitHashRate(hexutil.Uint64(100), common.HexToHash("a")); res {
		t.Error("expect to return false when submit hashrate to a stopped btpash")
	}
}
