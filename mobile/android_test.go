...
import (
	...
	"github.com/btpereum/go-btpereum/internal/build"
)
...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gbtp.LightScryptN, Gbtp.LightScryptP);
		...
		try {
			...
			// data from this mbtpod invocation is a JSON encoded, encrypted key-file.
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.btpereum", new NodeConfig());
			node.start();
			...
			btpereumClient ec = node.getbtpereumClient();
	...
	// signatures too. Regression test for go-btpereum issue #14599.
...
//
// This mbtpod has been adapted from golang.org/x/mobile/bind/java/seq_test.go/runTest
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogbtper
	...
	workspace, err := ioutil.TempDir("", "gbtp-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/btpereum/gbtptest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gbtp and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.btpereum", "github.com/btpereum/go-btpereum/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gbtp.aar"), "gbtp.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "btpereum", "gbtptest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
