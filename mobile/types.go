...
// Gbtpex retrieves the hex string representation of the block nonce.
func (n *Nonce) Gbtpex() string {
...
// Gbtpex retrieves the hex string representation of the bloom filter.
func (b *Bloom) Gbtpex() string {
	return fmt.Sprintf("0x%x", b.bloom[:])
}

// Header represents a block header in the btpereum blockchain.
type Header struct {
	header *types.Header
}
...
func (h *Header) Gbtpash() *Hash         { return &Hash{h.header.Hash()} }
...
// Block represents an entire block in the btpereum blockchain.
type Block struct {
	block *types.Block
}
...
func (b *Block) Gbtpash() *Hash                 { return &Hash{b.block.Hash()} }
func (b *Block) Gbtpeader() *Header             { return &Header{b.block.Header()} }
...
// Transaction represents a single btpereum transaction.
...
func (tx *Transaction) Gbtpash() *Hash   { return &Hash{tx.tx.Hash()} }
...
// Deprecated: use btpereumClient.TransactionSender
...
func (m *Message) Gbtpash() []byte     { return m.message.Hash }
