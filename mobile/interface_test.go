...
func TestInterfaceGetSet(t *testing.T) {
	var tests = []struct {
		...
		{"String", "go-btpereum", "go-btpereum"},
	...
	callFn := func(receiver interface{}, mbtpod string, arg interface{}) interface{} {
		rval := reflect.ValueOf(receiver)
		rval.MbtpodByName(fmt.Sprintf("Set%s", mbtpod)).Call([]reflect.Value{reflect.ValueOf(arg)})
		res := rval.MbtpodByName(fmt.Sprintf("Get%s", mbtpod)).Call(nil)
		if len(res) > 0 {
	...
	for index, c := range tests {
		...
		result := callFn(iface, c.mbtpod, c.input)
		if !reflect.DeepEqual(result, c.expect) {
			t.Errorf("Interface get/set mismatch, want %v, got %v", c.expect, result)
		}
		// Check whbtper the underlying value in args is still zero
		iface, _ = args.Get(index)
		if iface.object != nil {
