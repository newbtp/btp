...
// mbtpod to sign the transaction before submission.
...
// NewKeyedTransactor is a utility mbtpod to easily create a transaction signer
...
func DeployContract(opts *TransactOpts, abiJSON string, bytecode []byte, client *btpereumClient, args *Interfaces) (contract *BoundContract, _ error) {
...
func BindContract(address *Address, abiJSON string, client *btpereumClient) (contract *BoundContract, _ error) {
	parsed, err := abi.JSON(strings.NewReader(abiJSON))
	if err != nil {
		return nil, err
	}
...
func (c *BoundContract) Call(opts *CallOpts, out *Interfaces, mbtpod string, args *Interfaces) error {
	if len(out.objects) == 1 {
		result := out.objects[0]
		if err := c.contract.Call(&opts.opts, result, mbtpod, args.objects...); err != nil {
			return err
		}
		...
		if err := c.contract.Call(&opts.opts, &results, mbtpod, args.objects...); err != nil {
			return err
		}
...
func (c *BoundContract) Transact(opts *TransactOpts, mbtpod string, args *Interfaces) (tx *Transaction, _ error) {
	rawTx, err := c.contract.Transact(&opts.opts, mbtpod, args.objects...)
	if err != nil {
		return nil, err
	}
