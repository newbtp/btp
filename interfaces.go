...
// Package btpereum defines interfaces for interacting with btpereum.
package btpereum
...
// NotFound is returned by API mbtpods if the requested item does not exist.
...
// ChainReader provides access to the blockchain. The mbtpods in this interface access raw
...
type ChainReader interface {
	...
	// This mbtpod subscribes to notifications about changes of the head block of
	// the canonical chain.
...
type CallMsg struct {
	...
	Data     []byte          // input data, usually an ABI-encoded contract mbtpod invocation
}

// A ContractCaller provides contract calls, essentially transactions that are executed by
// the EVM but not mined into the blockchain. ContractCall is a low-level mbtpod to
...
type FilterQuery struct {
	BlockHash *common.Hash     // used by btp_getLogs, return logs only from block with this hash
...
// TransactionSender wraps transaction sending. The SendTransaction mbtpod injects a
// signed transaction into the pending transaction pool for execution. If the transaction
// was a contract creation, the TransactionReceipt mbtpod can be used to retrieve the
