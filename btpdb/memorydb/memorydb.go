...
// New returns a wrapped map with all the required database interface mbtpods
// implemented.
func New() *Database {
	return &Database{
...
// all the required database interface mbtpods implemented.
...
func (db *Database) NewBatch() btpdb.Batch {
	return &batch{
		db: db,
	}
}
...
func (db *Database) NewIterator() btpdb.Iterator {
...
// not exist).
func (db *Database) NewIteratorWithStart(start []byte) btpdb.Iterator {
	db.lock.RLock()
	defer db.lock.RUnlock()
...
func (db *Database) NewIteratorWithPrefix(prefix []byte) btpdb.Iterator {
	db.lock.RLock()
	defer db.lock.RUnlock()
...
//
// Note, this mbtpod is only used for testing (i.e. not public in general) and
...
func (b *batch) Replay(w btpdb.KeyValueWriter) error {
	for _, keyvalue := range b.writes {
		if keyvalue.delete {
...
// Next moves the iterator to the next key/value pair. It returns whbtper the
